...
int minimax(etat *e, int depth, int player) {
...
int main() {
    ...
    while (!isFull(currentstate.T) && !isWin(currentstate.T, 1) && !isWin(currentstate.T, 2)) {
        ...
        if (player == 1) {
            ...
            int bestmove = minimax(&currentstate, depth, 2);
